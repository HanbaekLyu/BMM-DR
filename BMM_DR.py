...
class ALS_DR():
    ...
    def __init__(self,
                 ...
                 subsample=True):
        '''
        BMM with diminishing radius for Nonnegative CP Tensor Factorization. Here for BMM we use proximal surrogates and 
        alpha is the proximal parameter.
        
        To recover Alternating Least Squares with Diminishing Radius, simply set alpha=0.
    ...
    def initialize_loading(self):
        ...
        for i in np.arange(self.n_modes):
            loading.update({'U' + str(i): np.random.normal(size=(self.X.shape[i], self.n_components))})
        return loading
    ...
    def update_code_within_radius(self, X, W, H0, r, alpha=0,
                                  ...
                                  subsample_ratio=None, nonnegativity=False,
                                  use_line_search=False):
        '''
        Find \hat{H} = argmin_H ( | X - WH|^2 + alpha/2*|H-H0|^2 ) within radius r from H0
        ...
        while (i < np.random.choice(sub_iter)):
            ...
            for k in [k for k in np.arange(H0.shape[0]) if if_continue[k]>0.5]:
                ...
                grad = (np.dot(A[k, :], H1) - B[k, :] + alpha * (H1[k,:]-H1_old[k,:]))
    ...
    def ALS(self,
        ...
        if ini_loading is not None:
            ...
            if nonnegativity:
                ...
                for i in np.arange(X.ndim):
                    loading.update({'U' + str(i): np.random.normal(size=(X.shape[i], self.n_components))})
        ...
        for step in trange(int(iter)):
            start = time.time()
            #alpha=alpha*0.3 #make alpha exponential to match diminishing radius
            for mode in np.arange(n_modes):
                ...
                if beta is None:  # usual nonnegative sparse coding
                    ...
                    Code = self.update_code_within_radius(X_new_mat, W, U.T, r=None, alpha=alpha,
    ...
    def MU(self,
           ...
           output_results=False):
        '''
        Given data tensor X and initial loading matrices W_ini, find loading matrices W by Multiplicative Update and
        the modified MUR.
        Ref: Shashua, Hazan, "Non-Negative Tensor Factorization with Applications to Statistics and Computer Vision" (2005)
        
        rho and delta are the threshold and regularization parameters in MUR.
        
        For MU, set both rho and delta to be zero. 
        '''
        print("delta=",delta)
        X = self.X
        ...
        for step in trange(int(iter)):
            ...
            for mode in np.arange(n_modes):
                ...
                U = np.maximum(U,delta)
                # print('!!! W.shape', W.shape)
                # print('!!! U.shape', U.shape)
                # print('!!! V.shape', V.shape)
                U_new = U.T * (W.T @ V + rho* U.T) / (W.T @ W @ U.T + rho* U.T)
