...
def plot_benchmark_errors(full_result_list, save_path):
    ...
    for i in np.arange(len(full_result_list)):
        ...
        rho = result_dict.get("rho")
        beta = result_dict.get("beta")
        alpha= result_dict.get("alpha")
        if delta==0:
            label0 = result_dict.get("method")
        elif delta!=0 and delta is not None:
            # label0 = result_dict.get("method") + " ($\\beta=${}, $c'=${:.0f})".format(beta, search_radius_const)
            label0 = result_dict.get("method") + " ($\\delta=${}, $\\rho=${})".format(delta,rho)
        if alpha==0:
            if beta==0 or beta is None:
                label0 = result_dict.get("method")
            else:
                label0 = result_dict.get("method") + " ($\\beta=${})".format(beta)
        elif alpha!=0 and alpha is not None:
            if beta==0 or beta is None:
                label0 = result_dict.get("method") + " ($\\lambda=${})".format(alpha)
            else:
                label0 = result_dict.get("method") + " ($\\beta=${}, $\\lambda=${})".format(beta, alpha)
    ...
    title = data_name
    plt.title(label=data_name ,loc="center",fontsize=15)
...
def main(n_components = 2,
        ...
        data_name = "Synthetic",#"Cifar10", "Synthetic", "MNIST" or "Twitter"
        nonnegativity=True,
        run_ALS = True,
        run_MU = True,
        run_MUR=True,
    ...
    if data_name == "Synthetic":
        ...
        X = np.load("data/mnist_1000x28x28.npy")
        select_data=np.random.randint(0,999)
        X= X[select_data,:,:]
        file_name="MNIST"
    ...
    if data_name =="Synthetic":
        search_radius_const = np.linalg.norm(X.reshape(-1,1),1) / 150000
    elif data_name =="Cifar10":
    ...
    if run_MUR:
        ...
        delta_list =[1e-8]
        rho_list =[1e-8]
        for (delta,rho) in zip(delta_list,rho_list):
            #if delta==0:
            #    rho=0
            #else:
            #    rho=1e-8
    ...
    if run_BMM_DR:
        ...
        for alpha in alpha_list:
            ...
            if alpha==0:
                ...
                results_dict.update({"method": "BMM-DR"})
            results_dict.update({"data_name": data_name})
